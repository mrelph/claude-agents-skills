...
**Sub-agent:** `spec-requirements` (in ${CLAUDE_PLUGIN_ROOT}/references/)
...
**Sub-agent:** `spec-design` (in ${CLAUDE_PLUGIN_ROOT}/references/)
...
**Sub-agent:** `spec-tasks` (in ${CLAUDE_PLUGIN_ROOT}/references/)

## Workflow Process

The skill follows a strict iterative workflow (see `${CLAUDE_PLUGIN_ROOT}/references/workflow-definition.md` for complete details):
...
This skill orchestrates multiple specialized sub-agents (all in `${CLAUDE_PLUGIN_ROOT}/references/`):
...
See `${CLAUDE_PLUGIN_ROOT}/references/` directory for:
