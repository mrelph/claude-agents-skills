...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/retirement_calculator.py --current-income <amount> --retirement-age <age> --lifestyle <basic|comfortable|affluent>
```
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/ss_optimizer.py --user-age <age> --spouse-age <age> --earnings-history <file>
```
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/tax_strategy.py --scenario roth-conversion --current-tax-bracket <bracket> --retirement-bracket <bracket>
```
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/monte_carlo.py \
...
python ${CLAUDE_PLUGIN_ROOT}/scripts/monte_carlo.py <params> > history/scenarios/base_case_$(date +%Y-%m-%d).json
```
...
**`${CLAUDE_PLUGIN_ROOT}/references/tax_strategies.md`** - Roth conversion analysis, withdrawal sequencing, bracket management

**`${CLAUDE_PLUGIN_ROOT}/references/healthcare_guide.md`** - Medicare enrollment, ACA marketplace, HSA strategies, long-term care

**`${CLAUDE_PLUGIN_ROOT}/references/ss_optimization.md`** - Claiming strategies, spousal benefits, survivor planning, earnings test
