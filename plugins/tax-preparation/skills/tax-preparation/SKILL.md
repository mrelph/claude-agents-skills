...
1. Extract relevant fields using the field-mapping tables in `${CLAUDE_PLUGIN_ROOT}/references/form_processing.md`
...
For RSU and stock compensation documents, always verify cost basis against vesting records. The 1099-B cost basis is frequently incorrect for RSUs. See `${CLAUDE_PLUGIN_ROOT}/references/form_processing.md` for detailed extraction procedures, brokerage platform import guidance, and the RSU verification process.
...
Collect all tax documents and personal information. Use the comprehensive checklist in `${CLAUDE_PLUGIN_ROOT}/references/document_checklist.md` to ensure nothing is missed.
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/deduction_analyzer.py --income <agi> --itemized-items <file.json>
```
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/credit_eligibility_checker.py --agi <amount> --filing-status <status> --dependents <info> --expenses <file.json>
```
...
See `${CLAUDE_PLUGIN_ROOT}/references/credits_guide.md` for detailed eligibility requirements, phaseout calculations, and interaction rules.
...
See `${CLAUDE_PLUGIN_ROOT}/references/self_employment_guide.md` for full guidance on deductions, retirement plan options, and recordkeeping.
...
Determine state filing requirements (resident, part-year, nonresident). Identify state-specific adjustments and credits. Calculate credit for taxes paid to other states in multi-state situations. See `${CLAUDE_PLUGIN_ROOT}/references/tax_planning.md` for state tax details.
...
Include planning recommendations for next year: withholding adjustments, retirement contribution optimization, Roth conversion opportunities, estimated payment scheduling, and charitable giving strategies. See `${CLAUDE_PLUGIN_ROOT}/references/tax_planning.md` for output formats and multi-year planning strategies.
...
| Script | Purpose |
|--------|---------|
| `${CLAUDE_PLUGIN_ROOT}/scripts/tax_calculator.py` | Core tax liability calculation |
| `${CLAUDE_PLUGIN_ROOT}/scripts/deduction_analyzer.py` | Standard vs itemized comparison |
| `${CLAUDE_PLUGIN_ROOT}/scripts/estimated_tax_calculator.py` | Quarterly payment calculations |
| `${CLAUDE_PLUGIN_ROOT}/scripts/credit_eligibility_checker.py` | Credit qualification analysis |
| `${CLAUDE_PLUGIN_ROOT}/scripts/tax_savings_finder.py` | Proactive deduction/credit identification |
| `${CLAUDE_PLUGIN_ROOT}/scripts/document_checklist_generator.py` | Personalized document checklists |
| `${CLAUDE_PLUGIN_ROOT}/scripts/rsu_calculator.py` | RSU cost basis, withholding, lot tracking, Form 8949 adjustments |

### Amazon RSU Scripts (under `${CLAUDE_PLUGIN_ROOT}/scripts/rsu/`)

| Script | Purpose |
|--------|---------|
| `${CLAUDE_PLUGIN_ROOT}/scripts/rsu/calculate_rsu_tax.py` | RSU tax calculation |
| `${CLAUDE_PLUGIN_ROOT}/scripts/rsu/calculate_cost_basis.py` | Cost basis computation |
| `${CLAUDE_PLUGIN_ROOT}/scripts/rsu/determine_holding_period.py` | Holding period analysis |
| `${CLAUDE_PLUGIN_ROOT}/scripts/rsu/generate_form_8949.py` | Form 8949 generation |
| `${CLAUDE_PLUGIN_ROOT}/scripts/rsu/parse_broker_csv.py` | Brokerage CSV parsing |
| `${CLAUDE_PLUGIN_ROOT}/scripts/rsu/verify_calculations.py` | Calculation verification |

### RSU Calculator Usage

```bash
# Calculate withholding shortfall
python ${CLAUDE_PLUGIN_ROOT}/scripts/rsu_calculator.py withholding --vesting-income 25000 --ytd-wages 75000 --filing-status married_jointly --state-rate 0.093

# Load and display lot summary
python ${CLAUDE_PLUGIN_ROOT}/scripts/rsu_calculator.py lots --vesting-file vestings.csv

# Calculate sale tax implications
python ${CLAUDE_PLUGIN_ROOT}/scripts/rsu_calculator.py sale --vesting-file vestings.csv --sale-date 2024-06-15 --shares 100 --sale-price 120.00 --reported-basis 0

# Quick cost basis calculation
python ${CLAUDE_PLUGIN_ROOT}/scripts/rsu_calculator.py basis --shares-vested 250 --fmv 100.00 --shares-withheld 87
```
...
| Reference | Contents |
|-----------|----------|
| `${CLAUDE_PLUGIN_ROOT}/references/tax_brackets_deductions.md` | Current year brackets, standard deductions, contribution limits, phaseout thresholds, AMT |
| `${CLAUDE_PLUGIN_ROOT}/references/credits_guide.md` | Detailed eligibility and calculations for all major tax credits |
| `${CLAUDE_PLUGIN_ROOT}/references/self_employment_guide.md` | Business deductions, quarterly payments, retirement options, recordkeeping |
| `${CLAUDE_PLUGIN_ROOT}/references/investment_taxes.md` | Capital gains, dividends, cost basis methods, wash sales, RSU/ISO/ESPP/NQSO treatment |
| `${CLAUDE_PLUGIN_ROOT}/references/overlooked_deductions.md` | Commonly missed deductions by situation (above-the-line, itemized, state-specific, carryforwards) |
| `${CLAUDE_PLUGIN_ROOT}/references/deduction_finder.md` | Proactive discovery checklists, probing questions, RSU-specific discovery, overlooked deduction tables |
