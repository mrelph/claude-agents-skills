...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/calculate_portfolio_metrics.py holdings.json > metrics.json
```
...
**Concern assessment**: Use `${CLAUDE_PLUGIN_ROOT}/references/analysis_framework.md` checklist - evaluate concentration, diversification, risk levels, inefficiencies, tax implications. Provide context and severity
...
```bash
mkdir -p history
python ${CLAUDE_PLUGIN_ROOT}/scripts/calculate_portfolio_metrics.py holdings.json > history/metrics_$(date +%Y-%m-%d).json
```
...
**`${CLAUDE_PLUGIN_ROOT}/references/analysis_framework.md`** - Load ONLY when user explicitly asks for concern assessment or needs systematic evaluation checklist

**`${CLAUDE_PLUGIN_ROOT}/references/market_benchmarks.md`** - Load ONLY when specific benchmark construction guidance is needed or the appropriate index is unclear
